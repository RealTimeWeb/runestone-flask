db.define_table('sections',
  ...
  Field('course_id', db.courses, label=('Course ID'), required=True),
  migrate='runestone_sections.table' if settings.migrate else False
  )
...
db.define_table('section_users',
    ...
    migrate= 'runestone_section_users.table' if settings.migrate else False
)
...
db.define_table('pipactex_deadline',
    ...
    migrate='runestone_pipactex_deadline.table' if settings.migrate else False)
